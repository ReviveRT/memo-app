 ...
 * -- rather than being a bare row and a bare array. MEMO-19 adds search to the list
 * route and will want to echo back what it matched on; an envelope has room for
 * that without changing the type of anything the frontend already reads, and a
 * top-level JSON array has none. The two keys are named rather than a shared
...
final class MemoController extends Controller
{
    ...
    {
        $memos = $this->memos->recent($request->limit());
