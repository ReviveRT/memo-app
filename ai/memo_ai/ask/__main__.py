...
def main() -> int:
    ...
    # The path *is* logged here, unlike in the worker's start-up line, and the
    # asymmetry is deliberate: the worker names the provider because a reader is
    # checking it against a .env, while this service has one job and the file it
    # opens is the interesting fact about whether it can do it.
    app = create_app(
        settings,
        ask_model.Model(
            settings.enrich_model_path,
